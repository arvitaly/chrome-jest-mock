...
export class Tabs {
    ...
    public create: ChromeJestMock.Tabs.Create = jest.genMockFunction().mockImplementation((props: chrome.tabs.CreateProperties, cb) => {
    ...
    public remove: ChromeJestMock.Tabs.Remove = jest.genMockFunction().mockImplementation((ids, cb) => {
        this.chrome.runtime.lastError = undefined;
    ...
    public get: ChromeJestMock.Tabs.Get = jest.genMockFunction().mockImplementation((tabId, callback) => {
        this.chrome.runtime.lastError = undefined;
        callback(this.tabs.filter((tab) => tab.id == tabId)[0]);
    });
    public executeScript: ChromeJestMock.Tabs.ExecuteScript = jest.genMockFunction().mockImplementation((tabId: number, details: chrome.tabs.InjectDetails, cb) => {
